...
func TestAssistanceNativeCallAndSharedAccounting(t *testing.T) {
	for _, variant := range []string{"luna", "sol", "deepseek-luna", "deepseek-sol"} {
		t.Run(variant, func(t *testing.T) {
			model := strings.TrimPrefix(variant, "deepseek-")
			effort := "none"
			if model == "sol" {
				effort = "low"
			}
			request := assistanceTestRequest(CognitiveProfile{Model: model, ReasoningEffort: effort}, "reasoning", false)
			if strings.HasPrefix(variant, "deepseek-") {
				selected := request.Config.CognitiveResource.Models[model]
				selected.ID = map[string]string{"luna": "deepseek-v4-flash", "sol": "deepseek-v4-pro"}[model]
				request.Config.CognitiveResource.Models[model] = selected
			}
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
				...
				if body["tools"].([]any)[0].(map[string]any)["strict"] != false {
					t.Error("helper did not use the unified portable function contract")
				}
