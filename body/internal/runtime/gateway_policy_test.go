...
func TestGatewayHTTPQuotaAndRateFactsRemainDistinct(t *testing.T) {
	for _, tc := range []struct {
		...
		{"provider_failure", `{"error":{"code":"provider_stream_failed"}}`, "upstream_unavailable", 502},
		{"function_disabled", `{"error":{"code":"function_call_not_supported"}}`, "function_call_not_supported", 422},
		{"effort_disabled", `{"error":{"code":"unsupported_reasoning_effort"}}`, "unsupported_reasoning_effort", 422},
	} {
...
func TestOnlyDocumentedLLMServerPreflightFailuresHaveKnownZeroCost(t *testing.T) {
	request := CognitiveRequest{Config: testConfig(10)}
	for _, adapter := range []string{"openai", "llmserver"} {
		request.Config.ModelGateway.Adapter = adapter
		for _, code := range []string{"function_call_not_supported", "unsupported_reasoning_effort", "http_error", "invalid_function_output"} {
			status := 422
			if code == "invalid_function_output" {
				status = 200
			}
			fact := modelFailureFact(request, code, status, "", "", "")
			known := adapter == "llmserver" && status == 422 && (code == "function_call_not_supported" || code == "unsupported_reasoning_effort")
			if (fact.CostStatus == "confirmed_zero") != known {
				t.Fatalf("manufactured or lost billing fact: %#v", fact)
			}
		}
	}
}
