...
func mentorWireHasAction(t *testing.T, request CognitiveRequest, kind string) bool {
	...
	for _, raw := range body["tools"].([]any) {
		...
		for _, candidate := range properties["action"].(map[string]any)["anyOf"].([]any) {
			action := candidate.(map[string]any)["properties"].(map[string]any)
			if action["kind"].(map[string]any)["enum"].([]any)[0] == kind {
				return true
			}
		}
	}
