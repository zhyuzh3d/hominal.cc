...
func isolatedModelInput(t *testing.T, request CognitiveRequest, inspect ...func(map[string]any)) string {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if request.Lease.ProfileSource == "next" {
			arguments = []byte(`{"answer":"synthetic local test answer"}`)
		}
		_ = json.NewEncoder(w).Encode(map[string]any{"model": "gpt-5.6-terra", "output": []map[string]any{{"type": "function_call", "name": name, "call_id": "test-call", "arguments": string(arguments)}}, "usage": map[string]int{"input_tokens": 1, "output_tokens": 1, "total_tokens": 2}})
	}))
	defer server.Close()
...
func testSchemaValue(schema map[string]any) any {
	...
	if alternatives, ok := schema["anyOf"].([]any); ok && len(alternatives) > 0 {
		return testSchemaValue(alternatives[0].(map[string]any))
	}
	switch schema["type"] {
	case "object":
		...
		for _, key := range schemaStringList(schema["required"]) {
			result[key] = testSchemaValue(properties[key].(map[string]any))
		}
		return result
	case "array":
		count := 0
		if minimum, ok := schemaNumber(schema["minItems"]); ok {
			count = int(minimum)
		}
		result := make([]any, count)
		items, _ := schema["items"].(map[string]any)
		for index := range result {
			result[index] = testSchemaValue(items)
		}
		return result
	case "string":
