...
func TestGatewayContractRejectionStaysWithItsCausalFocus(t *testing.T) {
	...
	for attempt := 1; attempt <= 4; attempt++ {
		...
		r.state.Usage = append(r.state.Usage, UsageRecord{Time: nowUTC(), LeaseID: lease, RequestedModel: "terra", ActualMicrousd: 1000, Status: "unusable", CostConfirmed: true, FailureCategory: "invalid_function_output", HTTPStatus: 200})
		failure := &ModelCallError{Fact: ModelFailureFact{Category: "invalid_function_output", HTTPStatus: 200}, Message: "provider returned an invalid function call"}
	...
	if r.state.Commitments[0].Status != "reality_available" || r.state.Usage[0].ActualMicrousd != 1000 || !r.state.Usage[0].CostConfirmed {
		t.Fatal("recovery lost Reality or the server's confirmed charge")
	}
...
func TestCapturedDeferredRealityReleasesOrganButNotUnknownOrDuplicateWork(t *testing.T) {
	r := recoveryRuntime(t)
	event := addRecoveryReality(r, "completed")
	markEventForRetry(&r.state, event.ID, "invalid")
	markEventForRetry(&r.state, event.ID, "invalid")
	if r.organHasUnabsorbedReality("browser") {
		t.Fatal("captured completed Reality still occupied the physical organ")
	}
	if err := r.validateUnabsorbedActionScope(CognitiveCommit{Action: CognitiveAction{Kind: "organ_action", OrganID: "browser", Operation: "browser_navigate", Input: `{"url":"https://example.test/new"}`}}, "other"); err != nil {
		t.Fatalf("a distinct causal thread could not use a released organ: %v", err)
	}
	oldEvent, _ := r.backgroundEvent(event.ID)
	var oldAction ActionState
	if err := json.Unmarshal(oldEvent.Payload, &oldAction); err != nil {
		t.Fatal(err)
	}
	duplicate := CognitiveAction{Kind: "organ_action", OrganID: oldAction.OrganID, Operation: oldAction.Operation, Input: oldAction.Request}
	if err := r.validateUnabsorbedActionScope(CognitiveCommit{Action: duplicate}, "other"); err == nil {
		t.Fatal("the exact still-unassimilated request was replayed as new work")
	}
	if err := r.validateUnabsorbedActionScope(CognitiveCommit{Action: CognitiveAction{Kind: "organ_action", OrganID: "system", Operation: "exec", Input: "pwd"}}, "concern-a"); err == nil {
		t.Fatal("the same concern opened a second action before assimilating its first result")
	}

	oldAction.Status = "unknown"
	payload, _ := json.Marshal(oldAction)
	for index := range r.state.Background {
		if r.state.Background[index].ID == event.ID {
			r.state.Background[index].Payload = payload
		}
	}
	if !r.organHasUnabsorbedReality("browser") {
		t.Fatal("an unknown outcome released the organ before its effects were established")
	}
}

func TestDeferredRealityRetryBackoffSpreadsRepeatedStructuralFailure(t *testing.T) {
	r := recoveryRuntime(t)
	event := addRecoveryReality(r, "completed")
	want := []time.Duration{20 * time.Second, 60 * time.Second, 3 * time.Minute, 9 * time.Minute, 10 * time.Minute}
	for attempt, expected := range want {
		markEventForRetry(&r.state, event.ID, "same invalid interpretation")
		markEventForRetry(&r.state, event.ID, "same invalid interpretation")
		current, _ := r.backgroundEvent(event.ID)
		if got := r.realityRetryDelay(current); got != expected {
			t.Fatalf("attempt group %d delay = %v, want %v", attempt, got, expected)
		}
		// Advance only one later attempt for the remaining sequence.
		if attempt < len(want)-1 {
			for index := range r.state.Background {
				if r.state.Background[index].ID == event.ID {
					r.state.Background[index].CognitionAttempts--
				}
			}
		}
	}
