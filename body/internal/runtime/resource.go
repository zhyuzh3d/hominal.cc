...
func resourceView(request CognitiveRequest, inputTokenEstimate int) map[string]any {
	...
	view := map[string]any{
		...
		"last_spend":        compactUsageView(request.State.CognitiveResource.LastSpend),
...
func compactUsageView(record *UsageRecord) any {
	if record == nil {
		return nil
	}
	return map[string]any{
		...
		"actual_microusd":  record.ActualMicrousd,
		"status":           record.Status,
		"cost_confirmed":   record.CostConfirmed,
