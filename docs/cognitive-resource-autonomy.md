...
5. 保护期间，尚未吸收的 Reality 保持同一因果身份并获得一次最低成本可用备用模型的 `resource_recovery`；这只恢复一次认知机会，不改变长期默认，也不替 alice 选择意义；
6. 备用也失败、没有备用或同一保护已经使用过恢复机会时进入 `model_wait`；主、备连续失败把原模型保护延长到完整十分钟，保留原焦点、Concern、已发生费用和失败事实，Pulse 与其他身体活动继续；
7. 三种模型共用同一供应商密钥时，密钥总额度耗尽会使所有模型同时不可用；模型切换不能解决这种物质断供，只有上游额度真实恢复才能恢复认知。
