...
同一机制也适用于 Luna/none、Terra/none、Sol/low 的模型配置。当前 Flash 同时担任低阶与主力，两者单价与底层模型相同，低阶的轻快来自较少上下文和较短输出；不能把角色名称当成已经测得的智力或速度差异。
...
`task:reasoning` 可用 luna/none 或 sol/low；`task:implementation` 使用 sol/low，并由主脑固定已认领行动的目标和必要内容。简单逻辑问题不必人为制造持续 Concern。`next` 配合 `action:none`，`purpose` 是主脑主动交出的材料；只有高阶可以通过 `include_self:true` 额外取到当前 Narrative Self。低阶输出上限 200 Token；高阶沿用配置的输出上限。实现协助另外获得真实身体操作契约。核心不自动附上全部个人回忆、情绪、关切或 Seed。
