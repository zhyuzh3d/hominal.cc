...
模型 API 的 function call 是主脑与认知模型之间的结构化传输，不等于器官动作。每次主意识只返回一份意义赋值、唯一焦点和至多一个行动，并关闭并行调用。全部 llmserver 模型使用同一个 Responses 路径和单个 `cognitive_commit` 契约；模型 ID 只改变能力、速度与价格，不改变 Alice 的因果规则。函数以 `strict:false` 传输，返回后由 Hominal 本地核对 JSON、函数身份、原始 Schema、当前状态和器官许可，随后才进入统一 CognitiveCommit、认知校验与动力学。局部助手使用同一函数机制；纯文本本能保持轻量路径。

Browser、System 和未来器官的全部 operations 不直接注册为模型工具；它们只作为身体事实进入认知上下文，再由内部 `CognitiveCommit.action` 形成行动意愿。内核验证候选身份、器官、operation 与参数后，才把动作交给 Organ Host。必要参数及事实约束只有一套，违规生成不自动补写、删改或执行。

原契约已经唯一确定的顶层元数据，如单候选焦点、固定空更新和零向量，可以由内核在本次事实现场绑定；生成时只声明仍有选择余地的字段，解码后再按同一请求还原完整结构并通过原始契约。动作参数、意义判断、多候选注意选择和允许的记忆、经验、叙事更新仍完整交给模型。这减少机械抄写和重复输入，不改变生命动力学的选择权。

这一边界避免模型绕过 AIP、Concern 和单线程注意直接操纵身体，也避免把全部器官动作 Schema 注入每次请求。llmserver 只传输 `function_call`，不执行任何器官，也不替 Hominal 批准或修复参数；现实、契约、状态与权限校验始终属于身体本地信任边界。
