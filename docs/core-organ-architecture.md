...
`capabilities` 是内核—器官协议能力，例如被动观察、定向、执行和取消；`operations` 是 Alice 可以在 `organ_action.operation` 中实际选择的行动目录。两者必须分开：内核可以调用 `observe`，不表示 Alice 应把 `observe` 猜成某个器官的行动名。具有 `perform` 能力的器官必须公布非空、无重复的 operations；Organ Host 在动作开始前按目录校验。器官可以为 operations 提供按需 Schema，但不能在 guidance 中赋予目标、推荐主题、规定行为顺序或替 Alice 评价内容。
...
- 网页快照解析、界面噪声过滤、稳定对象身份和 Direct URL；
