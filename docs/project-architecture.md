...
当前唯一快照创建于 2026-08-24 13:43，已经包含 `hominal` 用户、`/home/hominal` 和 `hominal-ThinkCentre` 主机名。回滚时从 SystemRescue 优盘启动，恢复 `/boot` 与 EFI 后合并 `system-baseline`；系统启动完成后自动重新创建同名的唯一快照。
...
这次演练验收了“优盘可启动”和“快照可被恢复环境正确识别”，没有执行 `lvconvert --merge`，因此没有用当前系统做破坏性的实际回滚。完整恢复仍应在允许丢弃当前根系统状态的专门演练中验收；在此之前，本文不把快照合并后的启动结果写成已证实事实。
