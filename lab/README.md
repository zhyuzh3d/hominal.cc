...
`python3 lab/contract_probe.py <请求JSON> --output <新的私有结果文件>`使用当前xconfig所选llmserver，每次仅一次真实付费调用，返回动作永不执行或导入个体。依赖本机`jsonschema`用于结果检查。`--inspect-schema`仅用于诊断：暂时允许对象通过服务后按原Schema列出差异，改变了生成约束，不能用作生产配置或历史失败的原文证据。正常调用失败或原契约违规返回非零退出码。结果文件包含个人上下文/输出，应保存在Lab私有目录，不提交仓库。

新版DeepSeek已明确fixed_shape_v1，旧`--inspect-schema`的anyOf诊断不适用。使用`deepseek_compatibility_probe.py`的guards、basic、cognition三个隔离阶段，传入`--frozen-request`与私有`--output-dir`，可指定`--model`、`--effort`及完整场景的`--repeat`。cognition仅在测试请求中将原action分支拆成独立函数，原字段约束保留；它不是生产适配器。每轮单独请求、无隐式重试、绝不执行返回动作；HTTP或本地Schema/预期不符返回非零退出码。成功也仅表示接口检查通过，不能替代运行时业务校验和真实生命实验。
