...
def prepare():
    ...
    with tempfile.TemporaryDirectory(prefix='hominal20-build-') as temp:
        ...
        for name in ['desktop.py','input_server.py','vision_server.py']:
...
def start(minutes):
    ...
    remote(shlex.join(['systemd-run','--user','--unit=hominal20-workbench','--collect',REMOTE+'/venv/bin/python',
                      REMOTE+'/tools/stage20_workbench.py','--workspace',REMOTE+'/workspaces/'+ident]))
    remote(shlex.join(['node',REMOTE+'/tools/stage20_cdp.mjs','location.href="http://127.0.0.1:8760/";true']))
    remote(shlex.join(['node',REMOTE+'/tools/stage20_cdp.mjs','fullscreen']))
...
def main():
    ...
    s.add_parser('prepare');a=s.add_parser('start');a.add_argument('--minutes',type=int,default=10)
