...
def main():
    p=argparse.ArgumentParser();p.add_argument('--rect');a=p.parse_args()
    ...
    script='''for (const w of workspace.windowList()) {if(w.caption.indexOf('Hominal20 工作室')>=0){%s const r=w.frameGeometry;callDBus('org.hominal20.Lab','/Lab','org.hominal20.Lab','Report',JSON.stringify({id:String(w.internalId),frame:[r.x,r.y,r.width,r.height]}));break;}}'''%mutation
