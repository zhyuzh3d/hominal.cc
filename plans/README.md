...
本目录保存从当前规范走向实现的阶段开发计划。计划可以被讨论和重写；已经冻结的产品概念进入 `docs/`，可执行出生输入进入 `genesis/`，真实实验结果进入 `lineage/`。

当前计划：

- [G0 阶段一：创生契约与架构冻结](./g0-stage-1-genesis-contract.md)（已完成）
...
- [G0 阶段八：自生关切与叙事自我成长](./g0-stage-8-self-originating-concern-and-narrative-growth.md)（已完成；同一冻结版本三个完整一小时样本）
